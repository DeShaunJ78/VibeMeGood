...
export default function StabilityRadar() {
  ...
  const grouped = {
    stable: (data ?? []).filter(r => r.volatilityRating === "stable"),
    elevated: (data ?? []).filter(r => r.volatilityRating === "elevated"),
    high: (data ?? []).filter(r => r.volatilityRating === "high" || r.volatilityRating === "boom_bust"),
  };
