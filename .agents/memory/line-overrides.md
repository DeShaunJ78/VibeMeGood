...
**Boundary guards:** the manual override path is NOT clamped like the auto-estimate, so the override endpoint enforces its own data-quality bounds (lineValueOverride |x|≤10000, payoutMultiplier 0.1–10). lineValueOverride mirrors raw `line_value`, which is legitimately negative for spreads/goblin lines — so bound MAGNITUDE, never force positivity. Keep the bound in sync across FOUR places: zod (pp-lines route), OpenAPI `PpLineOverrideInput`, the `data-quality` gate check, and the slate-board UI input guard (`Math.abs(v)<=10000`). A manual multiplier feeds optimizer EV directly with no downstream clamp.
